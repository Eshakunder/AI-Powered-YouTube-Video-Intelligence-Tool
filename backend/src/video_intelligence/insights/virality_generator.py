...
class ViralityScorer:
    ...
    def select_top(self, df, top_k=3, min_gap=3):
        """
        Return top K viral MOMENTS (timestamps, not text)
        """
        ...
        for idx, row in df.iterrows():
            ...
            # avoid nearby segments
            if any(abs(idx - u) < min_gap for u in used_indices):
                continue

            selected.append({
                "start": float(row['start']),
                "end": float(row.get('end', row['start'] + 5)),
                "score": round(float(row['virality_score']), 3)
            })
